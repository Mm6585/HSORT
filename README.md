...
> ID switching 문제를 해결하기 위해 학생의 위치 정보를 반고정으로 처리하는 방법을 시도하여 </br>
> 지정한 자리 좌표에 위치한 객체끼리의 겹침이 발생했을 때 ID switching이 발생하면, </br>
> DB에 저장된 ID를 겹침이 끝난 후에도 계속 그 자리에 위치한 객체에 부여하는 방법을 사용함. </br>
> 위 방법을 사용하여 문제를 완벽하게 해결하지는 못했지만 상기 서술했던 것처럼 완화하는 것에 성공함. </br>
* 보완해야할 점 </br>
Occlusion, ID switching 문제를 해결하기 위해서는 카메라의 설치 위치와 각도, 높이 등을 최적화 하는 방법 또는 </br>
여러 대의 카메라 영상을 stitching 하는 방법을 통해 영상 속 객체의 겹침과 차폐를 최소화하는 방법이 필요하다고 생각함. </br>
또한 출입 게이트의 처리 안정성, 처리속도를 향상 시키는 방법이 필요할 것으로 판단됨.
